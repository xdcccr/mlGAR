...
for(k in 1:N_repl){ # k=1
  ...
  y1_sigmoid.nlme = nlme(y1 ~ sigmoid(time, theta1, theta2, theta3),
                         ...
                         control = nlmeControl(maxIter = 10000, msMaxIter = 1000,
                                               pnlsMaxIter = 50, tolerance = 1e-4,
                                               returnObject = TRUE))
...
for(k in 1:N_repl){#k=1
  ...
  # mat_sd = nlme::pdMatrix(nlmeoutput$modelStruct$reStruct$id)
  # MCfile[k,c("Est_Level2Var_theta1",
  #            "Est_Level2Var_theta2",
  #            "Est_Level2Var_theta3")]=diag(mat_sd)
