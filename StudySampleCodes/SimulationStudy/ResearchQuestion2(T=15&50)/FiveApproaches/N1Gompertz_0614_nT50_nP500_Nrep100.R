...
nT = 50 # number of observations in time series 
...
theta10_start = 33 # asymptote, limitation when time gets close to +infinite
theta20_start = 3 # sets the displacement along the x-axis (translates the graph to the left or right).
...
#------------------- check MCfile before use it-------------------  
MCfile = read.csv(file_MCfile)
cols_to_replace <- c(EstParNames,
                     seParNames,
                     LLParNames,
                     ULParNames)

for(col in cols_to_replace){
  MCfile[, col][MCfile[, col] > 1000] <- NA
}
#------------------- Generate MCfileSumm-------------------  
MCfile = read.csv(file_MCfile)
