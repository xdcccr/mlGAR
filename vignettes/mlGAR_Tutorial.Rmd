...
For person $i$ at time $t$, the observation model is (Equation 1 in the paper):

$$
y_{t,i} = \tau_{t,i} + \eta_{t,i}
$$

where $\tau_{t,i}$ represents the trend component and $\eta_{t,i}$ represents the state fluctuation component.

**Trend Component (Gompertz Growth Curve, Equation 2):**
...
>
> - `MU[id,t]` corresponds to $\tau_{t,i}$ in the paper (Gompertz trend)
> - `MU_thetas[id,k]` corresponds to $\theta_{k,i}$ (person-specific Gompertz parameters)
...
generate_mlGAR_data <- function(nP = 150, nT = 15, seed = 123) {
  ...
  Y <- matrix(NA, nrow = nP, ncol = nT)          # Observed variable (y in paper)
  MU <- matrix(NA, nrow = nP, ncol = nT)         # Trend values (tau in paper)
  g <- matrix(NA, nrow = nP, ncol = nT)          # State fluctuations (eta in paper)
...
plot(simdata$time, simdata$g[sample_ids[1], ], type = "l", col = colors[1],
     ...
     xlab = "Time", ylab = expression(eta[t*","*i]), 
...
-   `MU[id,t]`: The Gompertz trend ($\tau_{t,i}$) for person `id` at time `t`, computed as $\theta_{1,i} \exp[-\theta_{2,i} \exp(-t \cdot \theta_{3,i})]$
-   `AR[id]*(Obs[t-1,2,id]-MU[id,t-1])`: The AR component captures carry-over from the previous deviation ($\eta_{t-1,i}$)
-   `1/IIV`: JAGS uses precision (inverse variance), so we specify `1/IIV` where `IIV` is the process noise variance ($\sigma^2_u$)

### Level-2 Model: Person-Specific Parameters

The person-specific parameters are drawn from normal distributions centered at the population means. Note that the individual AR coefficients (`AR[id]`) are **not** truncatedâ€”only the population mean (`Level2MeanAR`) has the stationarity constraint:
...
-   Asparouhov, T., & MuthÃ©n, B. (2010). Bayesian analysis using Mplus: Technical implementation. MuthÃ©n & MuthÃ©n.
-   Gates, K. M., Chow, S.-M., & Molenaar, P. C. M. (2023). *Intensive longitudinal analysis of human processes*. Chapman and Hall/CRC.
-   Ji, L., Chow, S.-M., Schermerhorn, A. C., Jacobson, N. C., & Cummings, E. M. (2020). Handling missing data in the modeling of intensive longitudinal data. *Structural Equation Modeling: A Multidisciplinary Journal*, 27(5), 715â€“736.
-   Li, Y., Oravecz, Z., Zhou, S., Bodber, Y., & Chow, S.-M. (2022). Bayesian forecasting with a regime-switching zero-inflated multilevel Poisson regression model: An application to adolescent alcohol use with selective missingness. *Psychometrika*, 87(3), 978â€“1008.
